$(document).ready(function () {
  var $googleProjectId = $('.google-project-id'),
    ...
    $googleRefreshBucketsSpinner = $googleRefreshBucketsBtn
      .parent()
      .next()
      .children(),
        ...
        if (textStatus == 'success') {
          if (response.error) {
            ...
            for (var i = 0; i < response.length; i++) {
              ...
              $googleBucketSelect.append(
                '<option value="' +
                  response[i].bucket +
                  '" data-url-prefix="' +
                  response[i].urlPrefix +
                  '">' +
                  response[i].bucket +
                  '</option>'
              );
  ...
  $('.google-expires-amount')
    .keyup(googleChangeExpiryValue)
    .change(googleChangeExpiryValue);
